...
├── pipeline-context.json           ← cross-pipeline context (auto-managed)
...
GHA-Dynamics repo
  ...
  [ ] pipeline-context.json NOT pre-committed (auto-generated by Pipeline 1 on first run)
```
...
1. Export from sandbox via `export-solution.yml` workflow
